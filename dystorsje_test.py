...
def calc_dev(dist_list, avr) -> float:
...
def quick_dystortion_check(t: tuple) -> int:
    
    d = 0
    
    v_tolerancy = 0.05 * t[0][0]
    h_tolerancy = 0.05 * t[1][0]
    
    for j in range(1, 3):
        #dystorsja beczkowa 
        if(t[0][0] - t[0][j] > v_tolerancy):
            d -= 1
        elif(t[1][0] - t[1][j] > h_tolerancy):
            d -= 1
            
        #dystorsja poduszkowa
        elif(t[0][0] - t[0][j] < -v_tolerancy):
            d += 1
        elif(t[1][0] - t[1][j] < -h_tolerancy):
            d += 1
...
def processing(n: int) -> list:
    ...
    #wspolczynnik od ktorego zalezy czy idziemy w lewo czy w prawo
    v_factor = -1
    #start point
    v_pointer = -1
    #iterator
    ...
    while(j <= x):
        ...
        white_passed = False
        #przesuwanie najpierw w lewo, potem w prawo
        ...
        for l in range(v_half):
            ...
            #wejscie na czarny prostokat
            if(px < rect_value and not white_passed):
                white_passed = True
            #znalezienie punktu start
            elif(px > back_value and white_passed):
                v_pointer = l
                break
        
        #jeśli nie znajdziemy punktu start
    ...
    v_middle = v_dist_left[0]
    v_far_left = v_dist_left[-1]
    v_far_right = v_dist_right[-1]
    ...
    h_middle = h_dist_up[0]
    h_far_up = h_dist_up[-1]
    h_far_down = h_dist_down[-1]
    
    vh_far_values = ((v_middle, v_far_left, v_far_right), (h_middle, h_far_up, h_far_down))
    
    quick_dystortion_check(vh_far_values)
